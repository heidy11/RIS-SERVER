 ...
 *
 * El equipo medico informa al PACS cuando empieza y cuando termina cada
 * estudio, mediante Modality Performed Procedure Step: N-CREATE al iniciar,
 * N-SET al finalizar. Ni el RIS ni este adapter escriben MPPS — lo escribe el
 * equipo. DCM4CHEE lo expone solo para consulta, asi que este job lo consulta
 * cada minuto y traduce ese estado al de la orden.
 *
 * Lo que resuelve: hoy alguien tiene que marcar a mano "en equipo" y
 * "terminado" en el RIS. Eso casi siempre se hace tarde o no se hace, y la
 * pantalla de recepcion deja de reflejar lo que realmente pasa en sala.
 */
...
/**
 * Los identificadores que unen el MPPS con la orden no estan en la raiz: viven
 * dentro de la Scheduled Step Attributes Sequence (0040,0270), que es la copia
 * que el equipo hace de la entrada de worklist que consumio.
 *
...
/**
 * Traduce el Performed Procedure Step Status (0040,0252) al estado del RIS.
 *
 * DISCONTINUED significa que el equipo abandono el estudio (el paciente se fue,
 * se cancelo a mitad). No se mapea a COMPLETED ni a CANCELED automaticamente:
 * es una situacion que alguien tiene que mirar, y decidirla sola seria peor
 * que dejarla visible. Se registra en mppsStatus y la orden no cambia.
 */
 ...
 *
 * Sin esta regla, un MPPS viejo que reaparezca en una consulta posterior podria
 * devolver a "en equipo" un estudio que el radiologo ya dio por terminado e
 * informado.
 */
...
function esAvance(actual, nuevo) {
  ...
  // Ambos estados tienen que estar en la escalera. Si el actual no lo esta
  // (CANCELED, por ejemplo), no hay avance posible: un MPPS tardio no debe
  // reactivar una orden que alguien cancelo a proposito.
  return i > -1 && j > -1 && j > i;
}
 ...
 *
 * Solo mira ordenes recientes que todavia no estan terminadas: un estudio que
 * el equipo nunca cerro no puede quedar consultandose para siempre.
 *
...
async function sincronizarEstados() {
  try {
    ...
    // Una sola consulta al PACS para todo el lote: pedir MPPS por estudio seria
    // una peticion por orden cada minuto.
    const mppsItems = await client.searchMpps({}, 1000);

    // Se indexa por Study UID y tambien por accession, porque no todos los
    // equipos completan los dos campos en la secuencia (0040,0270).
