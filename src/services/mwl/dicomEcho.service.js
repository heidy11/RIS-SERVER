 ...
 *
 * C-ECHO es el "ping" del protocolo DICOM: abre una asociacion, pregunta si
 * hay alguien del otro lado y cierra. Es el primer paso de las etapas 1B, 1C y
 * 1D del documento — antes de pelearse con la worklist hay que saber si el
 * equipo siquiera contesta.
 *
 * La diferencia con un ping de red es importante y conviene tenerla clara al
 * diagnosticar: un ping ICMP dice que la maquina esta encendida; un C-ECHO
 * exitoso dice ademas que el servicio DICOM esta levantado, que el puerto es el
 * correcto y que el equipo acepta el AE Title con el que nos presentamos. Los
 * tres son fallas distintas y este servicio las distingue en el mensaje.
 */
 ...
 * La libreria los entrega como numero, y "7" sin contexto no le dice nada a
 * nadie parado frente al equipo.
 */
...
/**
 * Comprueba que haya algo escuchando en el puerto TCP, antes de intentar la
 * asociacion DICOM.
 *
 * Se separan a proposito las dos capas: si el puerto no abre, el problema es de
 * red o de servicio y no tiene nada que ver con DICOM. Mezclarlas produce el
 * diagnostico mas inutil de todos — "no se pudo conectar" — cuando en realidad
 * se puede decir si el equipo esta apagado, si el puerto esta cerrado o si hay
 * un firewall en el medio.
 *
...
/**
 * Traduce el error tecnico a algo que le sirva a quien esta parado frente al
 * equipo con el cable en la mano.
 */
function explicarError(err) {
  // Los errores de socket a veces llegan sin mensaje y solo con `code`, asi que
  // se arma el texto con todo lo que haya antes de intentar clasificarlo.
  const msg = [
    err && err.message,
    err && err.code,
...
/**
 * Hace un C-ECHO contra un nodo DICOM.
 *
 * Nunca lanza: devuelve siempre un resultado describiendo que paso, porque
 * quien lo llama es una pantalla de diagnostico y un equipo apagado es una
 * respuesta valida, no un error del sistema.
 *
...
function asociarDicom({ host, port, calledAet, callingAet }) {
  return new Promise(resolve => {
    ...
    /**
     * El equipo contesto y rechazo la asociacion. Es un caso distinto de "no
     * responde" y hay que distinguirlo: aca la red esta bien y el equipo esta
     * encendido, lo que falla es la configuracion de AE Titles.
     *
     * Sin escuchar este evento, el rechazo pasaba desapercibido y el resultado
     * caia por timeout diciendo "puede estar apagado", que es justo lo contrario
     * de lo que pasa y manda a buscar el problema al lugar equivocado.
     */
