...
async function processEntry(entry) {
  ...
  try {
    ...
    if (entry.operation === 'DELETE') {
      ...
      // El paciente tiene que existir en el archivo antes de la entrada MWL o DCM4CHEE responde 404 "Patient[id=...] does not exist". Ambas llamadas son idempotentes, asi que un reintento repite las dos sin efectos raros.
    ...
    // Una orden cancelada se queda en DISABLED: ya no esta en la worklist y no tiene sentido mostrarla como "sincronizada" en la pantalla del RIS.
...
/** Un ciclo del worker: toma hasta workerBatchSize pendientes vencidos y los procesa en serie. */
...
async function runReconciliation() {
  try {
    ...
    // Se importa aqui y no arriba para evitar un ciclo de require entre el worker y el servicio de outbox.
