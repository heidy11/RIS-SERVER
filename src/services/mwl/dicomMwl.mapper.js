 ...
 *
 * Esta es la pieza con mas riesgo de errores sutiles de todo el adapter, y por
 * eso es una funcion pura: no toca la red ni la base de datos, y se puede
 * probar entera sin un DCM4CHEE levantado.
 *
 * Regla que causa la mayoria de las fallas de integracion (seccion 8 del
 * documento): Modality, Scheduled Station AE Title, la fecha y la hora NO son
 * atributos de nivel raiz. Viven dentro de la Scheduled Procedure Step
 * Sequence (0040,0100). Si se ponen en la raiz, DCM4CHEE acepta el POST con
 * 200 OK y el C-FIND del equipo no devuelve nada, sin ningun error visible.
 */
 ...
 *
 * Las consolas antiguas de CT/RX/eco suelen no negociar Specific Character Set
 * y muestran basura o directamente descartan la entrada si el nombre trae
 * tildes o enies. Nombres como "MUNOZ^JOSE MARIA" siempre se ven bien; los
 * acentos se pierden, que es preferible a que el paciente no aparezca.
 */
 ...
 *
 * Se usa hora local a proposito: la worklist tiene que coincidir con la fecha
 * que el tecnico ve en la consola del equipo, no con UTC. Un estudio agendado
 * a las 21:00 en La Paz cae en el dia siguiente si se formatea en UTC, y
 * entonces no aparece en la worklist de hoy.
 */
function toDA(date) {
...
/**
 * Construye el Patient Name en formato PN de DICOM:
 *   Apellidos^Nombres^SegundoNombre^Prefijo^Sufijo
 * Solo se usan los dos primeros componentes.
 */
 ...
 * Se valida antes de encolar, para que un dato faltante sea un error inmediato
 * y visible en el RIS en vez de un reintento infinito contra DCM4CHEE.
 *
...
function validateOrder(order) {
  ...
  if (!order.accessionNumber) {
    ...
    // El accession number NO se recorta: es el identificador que une la orden
    // del RIS con el estudio en el PACS. Si se envia mas largo, DCM4CHEE lo
    // guarda truncado y el estudio queda con un numero distinto al del RIS,
    // rompiendo la trazabilidad que exige la matriz de aceptacion. Es preferible
    // que la orden falle de forma visible a que se sincronice mal en silencio.
 ...
 * Se usa el accession number porque hay un unico SPS por orden y porque debe
 * ser estable entre reprogramaciones: DCM4CHEE hace upsert por
 * StudyInstanceUID + SPS ID, y un SPS ID nuevo crearia una entrada duplicada
...
/**
 * Traduce una orden del RIS a un objeto DICOM JSON listo para
 * POST /rs/mwlitems.
 *
 ...
 *
 * DCM4CHEE exige que el paciente exista en el archivo antes de aceptar la
 * entrada de worklist, asi que el worker hace un POST /rs/patients con esto
 * justo antes del POST /rs/mwlitems. Se deriva del mismo dataset en vez de
 * armarse aparte para que no puedan divergir: el paciente del archivo y el de
 * la worklist son, por construccion, los mismos valores.
 *
