...
const mwlController = {
  /** GET /api/mwl/health Estado del adapter: conectividad con DCM4CHEE, tamano de la cola y metricas del worker. */
  ...
  /** GET /api/mwl/orders/:accessionNumber/status Estado de sincronizacion de una orden concreta, con el ultimo error y el historial de intentos. */
  ...
  /** POST /api/mwl/orders/:accessionNumber/retry Reintento manual, para cuando soporte ya corrigio la causa (un AE Title mal configurado, el PACS que volvio) y no quiere esperar al proximo ciclo. */
  retry: async (req, res) => {
    try {
  ...
  /** GET /api/mwl/preview/:accessionNumber Devuelve el DICOM JSON exacto que se le enviaria a DCM4CHEE, sin enviarlo. */
  ...
  /** GET /api/mwl/worklist Lo que DCM4CHEE tiene realmente en la worklist ahora mismo. */
  ...
  /** POST /api/mwl/reconcile Dispara la reconciliacion a mano en vez de esperar al cron. */
  ...
  /** POST /api/mwl/mpps/sync Consulta ya mismo el estado que reportaron los equipos, sin esperar al proximo ciclo. */
  ...
  /** GET /api/mwl/mpps Lo que el PACS tiene registrado sobre estudios iniciados y terminados. */
  ...
  /** POST /api/mwl/equipment/:id/echo Verifica por DICOM si el equipo responde (C-ECHO). */
  echoEquipment: async (req, res) => {
    try {
      ...
      // Siempre 200: que el equipo este apagado es una respuesta valida del diagnostico, no un fallo de la peticion.
      res.json(resultado);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  ...
  /** POST /api/mwl/equipment/echo-all Prueba todos los equipos activos de una vez, mas el propio PACS. Pensado para la puesta en marcha: una pasada y se ve quien contesta. */
  echoAll: async (req, res) => {
    try {
      const equipos = await Equipment.find({ status: true });
      // En serie y no en paralelo: abrir una asociacion DICOM contra varios equipos a la vez confunde el diagnostico si la red es el problema.
      const resultados = [];
  ...
  /** GET /api/mwl/outbox?status=ERROR Cola de sincronizacion, para el panel de soporte. */
