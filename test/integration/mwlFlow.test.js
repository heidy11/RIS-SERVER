/**
 * Prueba de integracion del RIS-PACS Adapter: orden del RIS -> worklist real.
 *
 * A diferencia de las pruebas del mapeador, esta necesita infraestructura viva:
 *   - MongoDB en localhost:27017
 *   - el lab DCM4CHEE levantado:  yarn pacs-lab:up
 *
 * Si falta cualquiera de los dos, la prueba se salta en vez de fallar, para que
 * `yarn test` siga siendo verde en una maquina sin el lab.
 *
 * Usa una base de datos aparte (ris-mwl-test) y borra sus datos al terminar:
 * nunca toca la base del RIS.
 */
...
/**
 * Comprueba que la infraestructura este disponible antes de intentar nada.
 *
 * Si algo falta deja la conexion a Mongo cerrada antes de devolver el motivo:
 * una conexion abierta mantiene vivo el event loop y el proceso de pruebas se
 * queda colgado en vez de terminar reportando el salto.
 */
