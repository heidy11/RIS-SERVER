/**
 * Pruebas de la traduccion MPPS -> estado de la orden (Etapa 2).
 *
 * No necesitan infraestructura: se prueba la logica de lectura del dataset y
 * las reglas de transicion, que es donde estan las decisiones delicadas.
 */
...
/** Dataset MPPS como el que devuelve DCM4CHEE. */
function makeMpps({
  status = 'IN PROGRESS',
...
test('los identificadores se leen de la secuencia (0040,0270), no de la raiz', () => {
  // Es el mismo error de mapeo que en la worklist: buscarlos en la raiz
  // devuelve vacio y ninguna orden se actualizaria nunca.
  const mpps = makeMpps();
